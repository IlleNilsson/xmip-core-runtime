//! The runtime's side of `xmip_operate.h`: the table a surface calls, and
//! the operator's exports.
//!
...
//! snapshot and nothing else — no call here makes execution wait. What the
//! table stands for — its snapshot, what it holds, what is published — is
//! [`crate::operator`]'s, outside this folder.
//!
//! In `ffi/`, the one folder of the runtime that may hold unsafe code
//! (ADR-0050, refined 2026-09-25): a surface hands over `out` and `out_len`
//! and the header promises they are written.
#![allow(unsafe_code)]
...
use crate::operator::{Operator, announce_change, change_clock};
...
/// nothing yet still answers — with [`crate::start::unconfigured`], which is the truth about
